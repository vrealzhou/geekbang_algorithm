...
func TestIntKey(t *testing.T) {
	...
	var b IntKey = 2
	if a.Compare(b) != -1 {
		t.Errorf("Incorrect compare result %d", a.Compare(b))
	}
	if a.Compare(a) != 0 {
		t.Errorf("Incorrect compare result %d", a.Compare(b))
	}
...
func checkAVLTree(t *testing.T, n *AVLNode) (int, int) {
	if n == nil {
		return 0, 0
	}
	heightl, heightr := 0, 0
	v := int(n.Data.(IntKey))
	if n.Left != nil {
		vl, height := checkAVLTree(t, n.Left)
		if vl >= v {
			t.Errorf("Incorrect value on left %d and parent %d", vl, v)
		}
		heightl = height
	}
	// fmt.Printf("data: %d, height: %d, bal: %d\n", v, n.Height, n.Balance)
	if n.Right != nil {
		vr, height := checkAVLTree(t, n.Right)
		if vr <= v {
			t.Errorf("Incorrect value on right %d and parent %d", vr, v)
		}
		heightr = height
	}
	if heightl-heightr > 1 || heightl-heightr < -1 {
		t.Errorf("Unbalanced on left %d right %d", heightl, heightr)
	}
	return v, int(math.Max(float64(heightl), float64(heightr))) + 1
}
